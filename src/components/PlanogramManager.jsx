...
// HELPER: Extracted outside the component to satisfy strict ESLint purity rules
...
export default function PlanogramManager({ catalog, activeLocation, activeLocRunItems, upsertRunItem, refreshData, setIsAddingItem }) {
  ...
  const handleAddNewCooler = async () => {
    const newId = generateZoneId(); // Updated to use external helper
  ...
  return (
    <>
      ...
      <div className="space-y-6">
              ...
              <div className={`px-4 py-3 flex items-center justify-between border-b border-zinc-800 transition-colors ${isCollapsed ? 'bg-zinc-900/50' : activeLocation?.theme?.bgLight || 'bg-zinc-900'}`}>
                <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => !isGroupEditing && toggleGroup(groupName)}>
                  ...
                  {isGroupEditing ? (
                    <input 
                      ...
                      className="bg-zinc-950 border border-cyan-500/50 text-zinc-100 text-sm font-black px-2 py-1 rounded w-2/3 uppercase tracking-wider" 
                      autoFocus 
                    />
                  ) : (
                ...
                {isManagingLayout && (
                  <div className="flex items-center gap-3 ml-4 shrink-0">
                    {isGroupEditing ? (
                       <button onClick={() => saveGroupEdit(groupName)} className="text-emerald-400 hover:scale-110 transition-transform"><Save size={16}/></button>
                    ) : (
                       <button onClick={() => { setEditingGroup(groupName); setTempGroupName(groupName); }} className="text-zinc-500 hover:text-cyan-400"><Edit3 size={16}/></button>
                    )}
              ...
              {!isCollapsed && (
                <div className="p-3 space-y-4">
                  {zones.map((zone, shelfIndex) => {
                    ...
                    return (
                      <div key={zone.id} className="bg-zinc-900/40 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
                        
                        <div className="px-2 py-1 border-b border-zinc-800/50 flex items-center justify-between bg-zinc-950/30">
                        ...
                        <div className="p-2 bg-zinc-900/20">
                               ...
                               if (isManagingLayout) {
                                 return (
                                   <div key={index} className={`relative flex flex-col items-center justify-center text-center p-1 rounded border-2 h-16 transition-all cursor-pointer ${isThisTheSourceMove ? 'border-amber-500 bg-amber-950/40 shadow-[0_0_15px_rgba(245,158,11,0.4)] animate-pulse' : isSelectingForThisSlot ? 'border-cyan-500 bg-cyan-950/40 shadow-[0_0_15px_rgba(34,211,238,0.3)]' : item ? 'border-zinc-700 bg-zinc-800 hover:border-amber-500/50' : 'border-dashed border-zinc-700 hover:border-zinc-400'}`}
                                        onClick={() => handleSlotClick(zone, index, itemId)}>
                                      {item ? (
                                        <>
                                 ...
                                 <div key={index} className="flex flex-col h-full">
                                    {item ? (
                                       <button 
                                         ...
                                         onClick={() => handleQtyChange(itemId)} 
                                         className={`flex flex-col items-center justify-center text-center p-1 rounded border h-16 transition-all group ${isAlreadyStaged ? 'bg-zinc-900 border-zinc-800 opacity-50 cursor-not-allowed' : 'bg-zinc-800 border-zinc-700 hover:border-blue-500 hover:bg-blue-950/30 shadow-sm'}`}>
                                       ...
                                       <div className="flex items-center justify-center h-16 bg-zinc-950/50 border border-zinc-800 rounded opacity-40"></div>
                                    )}
                                 </div>
                               );
                             })}
                               ...
                               <div className="max-h-32 overflow-y-auto space-y-1 pr-1 grid grid-cols-2 gap-1 mb-2">
                                  {catalog.filter(i => i.name.toLowerCase().includes(slotSearchQuery.toLowerCase())).map(item => (
                                      <button key={item.id} onClick={() => assignItemToSlot(zone, activeSlot.index, item.id)} className="w-full text-left p-2 rounded-lg text-[10px] font-bold flex items-center justify-between border bg-zinc-900 text-zinc-400 border-zinc-800 hover:border-cyan-500 hover:text-cyan-400 transition-all">
                                        <span className="truncate pr-2">{item.name}</span> <Plus size={10} className="shrink-0" />
                                      </button>
                                  ))}
                               </div>
                               <button 
                                 onClick={() => setIsAddingItem(true)} 
                                 className="w-full py-2 border border-dashed border-emerald-500/50 text-emerald-400 rounded-lg text-[10px] font-black uppercase hover:bg-emerald-950/30 hover:border-emerald-400 transition-colors flex items-center justify-center gap-1">
                                 <Plus size={12} /> Create New Catalog Item
                               </button>
                             </div>
                           )}
                        </div>
                  ...
                  {isManagingLayout && (
                    <button onClick={() => handleAddNewShelf(groupName)} className="w-full py-3 border-2 border-dashed border-zinc-800 rounded-xl text-[10px] font-black uppercase text-zinc-500 hover:border-cyan-500/30 hover:text-cyan-400 transition-all">
                      + Add Shelf to {groupName}
                    </button>
                  )}
                </div>
              )}
        ...
        {isManagingLayout && (
          <button onClick={handleAddNewCooler} className="w-full py-4 border-2 border-dashed border-zinc-700 bg-zinc-900/50 rounded-2xl text-[11px] font-black uppercase text-zinc-400 hover:border-cyan-500/50 hover:text-cyan-400 transition-all">
            + Add New Cooler / Section
          </button>
        )}
      </div>
    </>
