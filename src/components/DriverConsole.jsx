...
export default function DriverConsole({ catalog, runItems, runPhase, setRunPhase, clockInDriver, updateItemStatus, finishRun }) {
    ...
    return (
      <div className="p-6 flex flex-col h-full items-center justify-center space-y-12 bg-zinc-950 font-sans relative overflow-hidden">
        ...
        <button 
          ...
          className="z-10 w-full max-w-sm bg-cyan-500 hover:bg-cyan-400 text-zinc-950 font-black uppercase py-5 rounded-2xl shadow-[0_0_25px_rgba(34,211,238,0.4)] flex items-center justify-center gap-3 transition-all transform active:scale-95 text-lg"
        >
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="p-8 text-center flex flex-col items-center justify-center flex-1 space-y-6">
                <div className="bg-emerald-500/10 border border-emerald-500/50 p-6 rounded-full text-emerald-400 animate-pulse">
                    <Check size={40}/>
                </div>
      ...
      <div className="flex flex-col h-full animate-in fade-in relative">
        ...
        <div className="border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md sticky top-[57px] z-20 shadow-lg">
          ...
          <div className="p-4 bg-zinc-950">
            ...
            <div className="relative">
                ...
                className="w-full p-4 rounded-xl bg-zinc-900 border border-zinc-700 text-cyan-400 font-black text-base focus:ring-2 focus:ring-cyan-500 outline-none appearance-none"
                >
                ...
                return (
                  <div key={item.id} className="bg-zinc-800 border border-zinc-700 rounded-xl p-3.5 shadow-sm flex items-center justify-between transition-all group active:scale-[0.98]">
                    ...
                    <div className="flex gap-2.5 ml-2">
                      <button onClick={() => updateItemStatus(item.id, GLOSSARY.system.itemStatus.SKIPPED)} className="p-3 rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-400 hover:bg-rose-950/30 hover:text-rose-400 hover:border-rose-500/50 transition-all flex items-center justify-center">
                        <X size={18} />
                      </button>
                      <button onClick={() => updateItemStatus(item.id, GLOSSARY.system.itemStatus.PROCURED)} className="p-3 rounded-xl bg-emerald-500 text-zinc-950 hover:bg-emerald-400 transition-colors shadow-[0_0_10px_rgba(16,185,129,0.2)] flex items-center justify-center">
                        <Check size={18} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="p-4 bg-zinc-950 border-t border-zinc-800 sticky bottom-0 z-10 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom-4">
          <button 
            ...
            onClick={() => setRunPhase(GLOSSARY.system.phases.DELIVERING)}
            className={`w-full py-4 rounded-xl font-black uppercase flex items-center justify-center gap-2 transition-all text-base ${pendingTotal === 0 ? 'bg-cyan-500 text-zinc-950 shadow-[0_0_20px_rgba(34,211,238,0.3)]' : 'bg-zinc-800 text-zinc-500 cursor-not-allowed border border-zinc-700'}`}
          >
            {pendingTotal > 0 ? `${pendingTotal} Items Remaining globally` : 'Proceed to Drop-offs'} <ArrowRight size={20} />
          </button>
        </div>
      </div>
    );
  ...
  if (runPhase === GLOSSARY.system.phases.DELIVERING) {
    const procuredItems = runItems.filter(i => i.status === GLOSSARY.system.itemStatus.PROCURED && i.itemId !== 'biz-card');
    
    return (
      <div className="flex flex-col h-full animate-in fade-in relative">
         <ShiftBanner shiftStart={shiftStart} />
         
         <div className="p-4 flex-1 space-y-4 pb-24 overflow-y-auto">
           <h2 className="text-xl font-black text-cyan-400 uppercase tracking-widest mb-4 mt-2">Route Drop-offs</h2>
           {Object.values(GLOSSARY.locations).map(loc => {
             const locItems = procuredItems.filter(i => i.locId === loc.id);
             if (locItems.length === 0) return null;
             
             return (
               <div key={loc.id} className="bg-zinc-800 rounded-2xl border border-zinc-700 overflow-hidden shadow-lg animate-in zoom-in-95">
                 <div className="bg-zinc-900/50 px-4 py-3 border-b border-zinc-700 flex items-center gap-3">
                   <loc.icon size={18} className={loc.theme.text} />
                   <h3 className="font-black text-zinc-100 uppercase text-sm tracking-tight">{loc.name}</h3>
                 </div>
                 <div className="p-2.5 space-y-0.5">
                   {locItems.map(item => {
                     const catItem = catalog.find(c => c.id === item.itemId);
                     return (
                       <div key={item.id} className="flex items-center justify-between p-2.5 border-b border-zinc-700/30 last:border-0">
         ...
         <div className="p-4 bg-zinc-950 border-t border-zinc-800 sticky bottom-0 z-10 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom-4">
            <button onClick={handleCompleteRun} className="w-full bg-emerald-500 text-zinc-950 font-black uppercase py-4 rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:bg-emerald-400 transition-colors text-base flex items-center justify-center gap-2 active:scale-[0.98]">
